...
export class WorkExperiencePageComponent implements OnInit {
  ...
  "Thrived in fast Agile teams, using Jira and Slack to ensure efficient communication in a remote setting."],
    details: {
      ...
      +'Another memorable ticked involved me <span class="underline">preventing cyber attacks</span> by improving the <span class="underline">file validation</span> on the frontend and backend. On the Angular side, I developed a new <span class="underline">validated-file-input</span> component in the internal shared library, which takes a <span class="underline">FormControl</span> and a list of approved file extensions as parameters. This gave me a little bit of experience developing <span class="underline">NPM Packages</span> as well as <span class="underline">custom Form Control validation</span>. For the backend, I developed an <span class="underline">injectable service</span> that will take an <span class="underline">IFormFile</span> object and a <span class="underline">File Type Enumerable</span>. It would then check the file extension, content-type and <span class="underline">file header bytes</span> of the IFormFile to make sure that it is the correct type. In order to make sure the code is as <span class="underline">reusable</span> and <span class="underline">extendable</span> as possible, I made use of the <span class="underline">strategy pattern</span> to deicide what <span class="underline">FileTypeDetails</span> class it would use to validate the IFormFile, based on the FileType Enumerable. This meant that in order to add a new file type for validation all the developer would have to do is add a new FileType value to the Enumerable and create a new FileTypeDetails file, <span class="underline">with no changes made to service itself</span>.<br><br>'
